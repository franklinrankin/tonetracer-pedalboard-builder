...
export function PedalCard({ pedal, isOnBoard = false, compact = false }: PedalCardProps) {
  ...
  if (compact) {
    return (
      <div 
        className={`group relative flex items-center gap-3 p-3 bg-white transition-all ${
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-black text-black truncate">{pedal.model}</span>
            {isOnBoard && (
              <span 
                className="px-1.5 py-0.5 text-[10px] font-black text-black bg-green-200"
                style={{ border: '2px solid black' }}
              >
                ON BOARD
              </span>
